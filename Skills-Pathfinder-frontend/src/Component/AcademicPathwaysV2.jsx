...
const AcademicPathwaysV2 = ({ user, onOpenCareerIntelligence, onBack }) => {
  ...
  const editSavedProfile = () => {
    ...
    setProfileForm({
      ...emptyProfile,
      ...savedProfile,
      interests: safeArray(savedProfile.interests).join(', '),
      expected_graduation_date: savedProfile.expected_graduation_date || ''
    });
  ...
  const saveGoal = async (event) => {
    ...
    try {
      ...
      setNotice({ type: 'success', message: 'Target career saved.' });
    } catch (error) {
    ...
    for (const skill of extractedSkills) {
      ...
      if (current) {
        ...
        const { data, error } = await supabase.from('skill_tracking').insert({
          ...
          metadata: { academic_profile_analysis_id: analysisId, sources: [sourceEntry] },
          last_seen_at: new Date().toISOString()
        }).select().single();
        if (error) throw error;
  ...
  const analyzeAcademicProfile = async () => {
    ...
    if (!profile?.field_of_study && !profile?.program_name && !subjects.length && !courses.length && !certificates.length) {
      setNotice({ type: 'error', message: 'Add an academic profile, subject, course, or certificate before generating Career Intelligence.' });
      return;
    }
    setAnalyzing(true);
    setNotice(null);
    try {
      ...
      const { data: analysis, error: analysisError } = await supabase.from('resume_analyses').insert({
        ...
        raw_analysis: { ...result, academic_profile: profile, academic_subjects: subjects, ongoing_courses: courses, certificates, career_goal: goal }
      }).select('id').single();
      ...
      setNotice({ type: 'success', message: `Academic evidence analyzed: ${extractedSkills.length} skills found. Opening Career Intelligence…` });
  ...
  const completedNames = new Set(subjects.filter((item) => item.status === 'completed').map((item) => normalize(item.subject_name)));
  const suggestedSubjects = useMemo(() => {
    const target = `${goal.career_title} ${savedProfile?.field_of_study || ''}`.toLowerCase();
    const ideas = target.includes('data') ? ['Statistics', 'SQL', 'Database Systems', 'Data Visualization', 'Programming with Python']
      : target.includes('biology') || target.includes('health') ? ['Statistics for Life Sciences', 'Genetics', 'Biochemistry', 'Research Methods', 'Cell Biology']
      : target.includes('cyber') ? ['Computer Networks', 'Linux', 'Operating Systems', 'Cloud Security', 'Programming']
      : ['Statistics', 'Research Methods', 'Professional Communication', 'Digital Literacy', 'Project-Based Learning'];
    return ideas.filter((item) => !completedNames.has(normalize(item))).slice(0, 5);
  }, [goal.career_title, savedProfile?.field_of_study, subjects]);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 p-6 text-white shadow-xl sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-300">Academic study pathway</p><h2 className="mt-2 text-3xl font-black">Academic Profile, Subjects & Career Goal</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Use this area for current study and future education planning. It works together with your employment profile and resume.</p></div><button onClick={onBack} className="rounded-xl border border-white/15 px-4 py-2 text-sm font-bold">Back to Dashboard</button></div></section>
