...
const AcademicPathwaysV3 = ({ user, onOpenCareerIntelligence, onBack }) => {
  ...
  const buildEvidenceSkills = () => {
    const map = new Map();
    const add = (name, category = 'Evidence', confidence = 0.75, source = 'academic_profile') => {
      const clean = String(name || '').trim();
      if (!clean) return;
      const key = normalize(clean);
      const current = map.get(key);
      if (!current || Number(current.confidence || 0) < confidence) map.set(key, { name: clean, category, confidence, source });
    };

    subjects.forEach((item) => {
      safeArray(item.skills_learned).forEach((skill) => add(skill, item.subject_area || 'Academic Skill', item.status === 'completed' ? 0.86 : 0.68, 'academic_subject'));
      add(item.subject_name, item.subject_area || 'Academic Subject', item.status === 'completed' ? 0.72 : 0.58, 'academic_subject');
    });
    courses.forEach((item) => {
      safeArray(item.extracted_skills).forEach((skill) => add(skill?.name || skill, item.subject_area || 'Course Skill', 0.78, 'ongoing_course'));
      add(item.course_name, item.subject_area || 'Ongoing Course', item.status === 'completed' ? 0.74 : 0.62, 'ongoing_course');
    });
    certificates.forEach((item) => {
      safeArray(item.extracted_skills).forEach((skill) => add(skill?.name || skill, skill?.category || 'Certificate Skill', item.is_verified ? 0.96 : 0.8, 'certificate'));
      safeArray(item.subjects).forEach((subject) => add(subject, 'Certificate Subject', item.is_verified ? 0.9 : 0.74, 'certificate'));
    });
    if (savedProfile?.field_of_study) add(savedProfile.field_of_study, 'Field of Study', 0.7, 'academic_profile');
    if (savedProfile?.program_name) add(savedProfile.program_name, 'Academic Program', 0.64, 'academic_profile');
    safeArray(savedProfile?.interests).forEach((interest) => add(interest, 'Career Interest', 0.55, 'academic_profile'));
    return Array.from(map.values());
  };

  const persistBlueprint = async (careerTitle, payload) => {
    if (!user?.id || !careerTitle || !payload?.blueprint) return;
    const key = normalize(careerTitle).replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'target';
    const { error } = await supabase.from('career_findings').upsert({
      user_id: user.id,
      client_record_key: `career-blueprint:${key}`,
      finding_type: 'career_blueprint',
      source_type: payload.blueprint.source_type || 'career_blueprint_api',
      title: `Career blueprint: ${payload.blueprint.canonical_title || careerTitle}`,
      status: 'active',
      data: payload,
  ...
  const fetchBlueprint = async (careerTitle, { persist = true, quiet = false } = {}) => {
    const title = String(careerTitle || '').trim();
    if (!title) {
      setBlueprint(null);
      return null;
    }
    if (!quiet) setBlueprintLoading(true);
    try {
      const response = await fetch(`${apiBase()}/api/career-blueprint`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ career_title: title, skills: buildEvidenceSkills() })
      });
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
        throw new Error(body.detail || `Career blueprint returned ${response.status}`);
      }
      const payload = await response.json();
      setBlueprint(payload.blueprint || null);
      if (persist && payload.blueprint) await persistBlueprint(title, payload);
      return payload;
    } catch (error) {
      if (!quiet) setNotice({ type: 'error', message: `Career blueprint could not be prepared: ${error.message}` });
      return null;
    } finally {
      if (!quiet) setBlueprintLoading(false);
    }
  };
  ...
  const load = async () => {
    ...
    try {
      ...
      if (goalResult.data?.career_title) {
        setTimeout(() => fetchBlueprint(goalResult.data.career_title, { persist: false, quiet: true }), 0);
      }
    } catch (error) {
  ...
  const saveGoal = async (event) => {
    ...
    try {
      ...
      await fetchBlueprint(data.career_title, { persist: true });
      setNotice({ type: 'success', message: `Target career saved. Skills Pathfinder prepared a career blueprint for ${data.career_title}.` });
    } catch (error) {
  ...
  const addSuggestedSubject = async (suggestion) => {
    try {
      const { data, error } = await supabase.from('academic_subjects').insert({
        ...
        status: 'planned',
        skills_learned: [],
        notes: suggestion.reason || `Suggested for ${goal.career_title || 'career development'}`
      }).select().single();
      if (error) throw error;
      setSubjects((current) => [data, ...current]);
      setNotice({ type: 'success', message: `${suggestion.name} added to planned subjects.` });
    } catch (error) {
  ...
  const saveCourse = async (event) => {
    ...
    setNotice(null);
    try {
      const extractedSkills = commaList(courseForm.skills).map((name) => ({ name, category: 'Course Evidence', confidence: 1, source: 'ongoing_course' }));
  ...
  const persistAcademicSkills = async (analysisId, extractedSkills) => {
    ...
    for (const skill of extractedSkills) {
      ...
      const sourceEntry = { source: 'academic_profile', source_record_id: analysisId, verification_status: 'ai_verified', evidence: 'Academic profile, subjects, courses and certificate evidence' };
      if (current) {
  ...
  const analyzeAcademicProfile = async () => {
    ...
    try {
      ...
      let recommendations = safeArray(result.recommendations);
      let targetPayload = null;
      if (goal.career_title?.trim()) {
        const targetResponse = await fetch(`${apiBase()}/api/target-career-analysis`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ career_title: goal.career_title.trim(), skills: extractedSkills })
        });
        if (targetResponse.ok) {
          targetPayload = await targetResponse.json();
          if (targetPayload?.blueprint) {
            setBlueprint(targetPayload.blueprint);
            await persistBlueprint(goal.career_title.trim(), targetPayload);
          }
          if (targetPayload?.recommendation) {
            const target = targetPayload.recommendation;
            recommendations = [target, ...recommendations.filter((item) => item.id !== target.id && normalize(item.path) !== normalize(target.path))].slice(0, 6);
          }
        }
      }
      ...
      const { data: analysis, error: analysisError } = await supabase.from('resume_analyses').insert({
        ...
        raw_analysis: { ...result, recommendations, target_career_blueprint: targetPayload?.blueprint || blueprint, academic_profile: profile, academic_subjects: subjects, ongoing_courses: courses, certificates, career_goal: goal }
      }).select('id').single();
      ...
      const { error: findingError } = await supabase.from('career_findings').upsert({
        ...
        status: 'active',
        data: { academic_profile: profile, academic_subjects: subjects, ongoing_courses: courses, career_goal: goal, extracted_skills: extractedSkills, recommendations, target_career_blueprint: targetPayload?.blueprint || blueprint },
        updated_at: new Date().toISOString()
      }, { onConflict: 'user_id,client_record_key' });
      ...
      const preferred = {
        ...
        target_career_blueprint: targetPayload?.blueprint || blueprint
      };
      setNotice({ type: 'success', message: `Academic evidence analyzed: ${extractedSkills.length} skills found and ${recommendations.length} career paths prepared.` });
  ...
  const existingNames = new Set([
    ...subjects.filter((item) => ['completed', 'planned', 'in_progress'].includes(normalize(item.status))).map((item) => normalize(item.subject_name)),
    ...courses.filter((item) => ['in_progress', 'active', 'planned', 'completed'].includes(normalize(item.status))).map((item) => normalize(item.course_name))
  ]);
  const suggestedSubjects = useMemo(() => safeArray(blueprint?.recommended_subjects)
    .filter((item) => item?.name && !existingNames.has(normalize(item.name)))
    .slice(0, 6), [blueprint, subjects, courses]);
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 p-6 text-white shadow-xl sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-300">Unified academic pathway</p><h2 className="mt-2 text-3xl font-black">Academic Profile, Learning & Career Goal</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Use any evidence you have. Your academic record, current courses, certificates, skills and optional resume all feed one Career Intelligence profile.</p></div><button onClick={onBack} className="rounded-xl border border-white/15 px-4 py-2 text-sm font-bold">Back to Dashboard</button></div></section>
    ...
    <div className="grid gap-6 xl:grid-cols-3">
      <form onSubmit={saveProfile} className="app-card p-6 xl:col-span-2"><h3 className="text-xl font-bold text-slate-900">{savedProfile ? 'Update academic profile' : 'Add academic profile'}</h3><p className="mt-1 text-sm text-slate-500">After saving, these fields clear. Your saved profile remains above.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{[
      ...
      ].map(([field,label,type]) => <label key={field} className="text-sm font-semibold text-slate-700">{label}<input type={type} step={field === 'gpa' ? '0.01' : '0.5'} value={profileForm[field] ?? ''} onChange={(event) => setProfileForm({ ...profileForm, [field]: event.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 font-normal" /></label>)}<label className="md:col-span-2 text-sm font-semibold text-slate-700">Interests<input value={profileForm.interests || ''} onChange={(event) => setProfileForm({ ...profileForm, interests: event.target.value })} placeholder="biology, aviation, healthcare, research, technology" className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 font-normal" /></label></div><button disabled={saving} className="mt-5 rounded-xl bg-teal-600 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{saving ? 'Saving…' : 'Save academic profile'}</button></form>

      <form onSubmit={saveGoal} className="rounded-2xl border border-teal-200 bg-teal-50 p-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-teal-700">Primary target</p><h3 className="mt-1 text-xl font-bold text-slate-900">Career goal</h3><input value={goal.career_title || ''} onChange={(event) => setGoal({ ...goal, career_title: event.target.value })} placeholder="Commercial Pilot, Nurse, Data Scientist..." className="mt-4 w-full rounded-xl border border-teal-200 bg-white px-3 py-2.5" /><label className="mt-3 block text-xs font-semibold text-teal-800">Target career date<input type="date" value={goal.target_date || ''} onChange={(event) => setGoal({ ...goal, target_date: event.target.value })} className="mt-1 w-full rounded-xl border border-teal-200 bg-white px-3 py-2.5" /></label><textarea rows={4} value={goal.motivation || ''} onChange={(event) => setGoal({ ...goal, motivation: event.target.value })} placeholder="Why are you interested in this goal?" className="mt-3 w-full rounded-xl border border-teal-200 bg-white p-3" /><button disabled={saving || blueprintLoading || !goal.career_title.trim()} className="mt-3 w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{blueprintLoading ? 'Preparing career blueprint…' : 'Save career goal'}</button></form>
    </div>

    {blueprint && <section className="rounded-3xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-indigo-700">Target Career Blueprint</p><h3 className="mt-1 text-2xl font-black text-slate-950">{blueprint.canonical_title}</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{blueprint.career_summary}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${blueprint.regulated_role ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'}`}>{blueprint.regulated_role ? 'Regulated / official requirements may apply' : 'Career pathway'}</span></div>{blueprint.regulation_note && <p className="mt-4 rounded-xl bg-white p-3 text-xs leading-5 text-slate-600">{blueprint.regulation_note}</p>}<div className="mt-5 grid gap-4 md:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Core competencies</p><div className="mt-2 flex flex-wrap gap-2">{safeArray(blueprint.core_competencies).map((item) => <span key={item} className="rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-800">{item}</span>)}</div></div><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Education / training pathway</p><div className="mt-2 space-y-2">{safeArray(blueprint.education_or_training_pathway).map((item) => <p key={item} className="rounded-lg bg-white px-3 py-2 text-xs text-slate-700">{item}</p>)}</div></div></div></section>}

    <div className="grid gap-6 xl:grid-cols-3"><form onSubmit={addSubject} className="app-card p-6"><h3 className="text-xl font-bold">Add academic subject</h3><div className="mt-4 space-y-3"><input required placeholder="Subject name" value={subjectForm.subject_name} onChange={(event) => setSubjectForm({ ...subjectForm, subject_name: event.target.value })} className="w-full rounded-xl border px-3 py-2.5" /><div className="grid grid-cols-2 gap-3"><input placeholder="Code" value={subjectForm.subject_code} onChange={(event) => setSubjectForm({ ...subjectForm, subject_code: event.target.value })} className="rounded-xl border px-3 py-2.5" /><input type="number" step="0.5" placeholder="Credits" value={subjectForm.credit_hours} onChange={(event) => setSubjectForm({ ...subjectForm, credit_hours: event.target.value })} className="rounded-xl border px-3 py-2.5" /></div><input placeholder="Subject area" value={subjectForm.subject_area} onChange={(event) => setSubjectForm({ ...subjectForm, subject_area: event.target.value })} className="w-full rounded-xl border px-3 py-2.5" /><input placeholder="Semester" value={subjectForm.semester} onChange={(event) => setSubjectForm({ ...subjectForm, semester: event.target.value })} className="w-full rounded-xl border px-3 py-2.5" /><select value={subjectForm.status} onChange={(event) => setSubjectForm({ ...subjectForm, status: event.target.value })} className="w-full rounded-xl border px-3 py-2.5"><option value="completed">Completed</option><option value="in_progress">In progress</option><option value="planned">Planned</option></select><input placeholder="Skills learned, comma separated" value={subjectForm.skills_learned} onChange={(event) => setSubjectForm({ ...subjectForm, skills_learned: event.target.value })} className="w-full rounded-xl border px-3 py-2.5" /></div><button className="mt-4 w-full rounded-xl bg-teal-600 px-4 py-3 text-sm font-bold text-white">Add subject</button></form><div className="app-card p-6 xl:col-span-2"><h3 className="text-xl font-bold">Academic record</h3><p className="text-sm text-slate-500">{subjects.length} subjects · {recordedCredits} recorded credits</p><div className="mt-5 space-y-3">{subjects.map((subject) => <div key={subject.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4"><div><p className="font-bold text-slate-900">{subject.subject_name}</p><p className="text-xs text-slate-500">{subject.semester || 'Semester not set'} · {subject.credit_hours || 0} credits · {subject.status}</p></div><button type="button" onClick={() => removeSubject(subject.id)} className="text-xs font-bold text-rose-600">Remove</button></div>)}{subjects.length === 0 && <p className="rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">No subjects recorded yet.</p>}</div></div></div>

    <section className="app-card p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-700">Ongoing courses</p><h3 className="mt-1 text-xl font-bold text-slate-950">Add current learning here</h3><p className="mt-1 text-sm text-slate-500">Courses are optional and are included automatically in skill-gap and readiness analysis.</p></div><span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-700">{courses.length} saved</span></div><form onSubmit={saveCourse} className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-4"><input required placeholder="Course name" value={courseForm.course_name} onChange={(event) => setCourseForm({ ...courseForm, course_name: event.target.value })} className="rounded-xl border px-3 py-2.5" /><input placeholder="Institution / provider" value={courseForm.institution} onChange={(event) => setCourseForm({ ...courseForm, institution: event.target.value })} className="rounded-xl border px-3 py-2.5" /><input placeholder="Subject area" value={courseForm.subject_area} onChange={(event) => setCourseForm({ ...courseForm, subject_area: event.target.value })} className="rounded-xl border px-3 py-2.5" /><input type="number" step="0.5" placeholder="Credits" value={courseForm.credit_hours} onChange={(event) => setCourseForm({ ...courseForm, credit_hours: event.target.value })} className="rounded-xl border px-3 py-2.5" /><input placeholder="Semester" value={courseForm.semester} onChange={(event) => setCourseForm({ ...courseForm, semester: event.target.value })} className="rounded-xl border px-3 py-2.5" /><label className="text-xs font-semibold text-slate-600">Expected course completion<input type="date" value={courseForm.expected_completion_date} onChange={(event) => setCourseForm({ ...courseForm, expected_completion_date: event.target.value })} className="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm" /></label><select value={courseForm.status} onChange={(event) => setCourseForm({ ...courseForm, status: event.target.value })} className="rounded-xl border px-3 py-2.5"><option value="in_progress">In progress</option><option value="planned">Planned</option><option value="completed">Completed</option></select><input placeholder="Skills covered, comma separated" value={courseForm.skills} onChange={(event) => setCourseForm({ ...courseForm, skills: event.target.value })} className="rounded-xl border px-3 py-2.5" /><button disabled={saving} className="rounded-xl bg-violet-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-50">Save ongoing course</button></form>{courses.length ? <div className="mt-5 grid gap-3 md:grid-cols-2">{courses.map((course) => <div key={course.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-slate-900">{course.course_name}</p><p className="mt-1 text-xs text-slate-500">{course.institution || course.provider || 'Institution/provider not set'} · {course.subject_area || 'Subject area not set'} · {course.status || 'in_progress'}</p>{course.expected_completion_date && <p className="mt-1 text-xs font-semibold text-violet-700">Expected course completion: {course.expected_completion_date}</p>}</div><button type="button" onClick={() => removeCourse(course.id)} className="text-xs font-bold text-rose-600">Remove</button></div></div>)}</div> : <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No ongoing courses are saved yet. Add one above if applicable. This is optional.</p>}</section>

    <section className="rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-50 to-white p-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-sky-700">Next semester planning</p><h3 className="mt-1 text-2xl font-black text-slate-900">Subjects to consider for {blueprint?.canonical_title || goal.career_title || savedProfile?.field_of_study || 'your goal'}</h3><p className="mt-2 text-sm text-slate-600">These suggestions come from the target career blueprint, not from a fixed list. They exclude subjects and courses already recorded. They are guidance, not official degree requirements.</p>{blueprintLoading && <p className="mt-4 text-sm text-slate-500">Preparing career-specific suggestions…</p>}<div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{suggestedSubjects.map((item, index) => <div key={item.name} className="flex flex-col rounded-2xl border border-sky-100 bg-white p-4"><span className="text-xs font-bold text-sky-700">OPTION {index + 1}</span><p className="mt-1 font-bold text-slate-900">{item.name}</p><p className="mt-2 flex-1 text-xs leading-5 text-slate-500">{item.reason || 'Supports preparation for this target career.'}</p><button type="button" onClick={() => addSuggestedSubject(item)} className="mt-4 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs font-bold text-sky-800">Add to planned subjects</button></div>)}</div>{!blueprint && goal.career_title && <button type="button" onClick={() => fetchBlueprint(goal.career_title)} className="mt-4 rounded-xl border border-sky-200 bg-white px-4 py-2 text-sm font-bold text-sky-800">Prepare target-career blueprint</button>}{blueprint && suggestedSubjects.length === 0 && <p className="mt-4 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-800">The current suggested subjects are already represented in your saved subjects or courses.</p>}<div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={analyzeAcademicProfile} disabled={analyzing} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{analyzing ? 'Analyzing saved evidence…' : 'Generate Career Intelligence'}</button><button type="button" onClick={() => onOpenCareerIntelligence?.()} className="rounded-xl bg-teal-600 px-5 py-3 text-sm font-bold text-white">Open Latest Career Intelligence</button></div></section>
  </div>;
};
