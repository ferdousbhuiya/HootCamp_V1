    ...
    {bestCareer && <>
      ...
      <section className="space-y-4">{rankedRecommendations.map((rec,index)=>{
        ...
        return <article key={rec.id||rec.path} className={`app-card overflow-hidden ${index===0?'border-indigo-200':''}`}>
          ...
          {open&&<div className="border-t bg-slate-50/60 p-6">
            <div className="flex gap-2 overflow-x-auto">{['overview','market','certifications','degrees','next-steps','resources'].map((tab)=><button key={tab} onClick={()=>setActiveTab(tab)} className={`shrink-0 rounded-lg px-3 py-2 text-sm font-semibold ${activeTab===tab?'bg-indigo-600 text-white':'bg-white text-slate-600'}`}>{tab==='next-steps'?'roadmap':tab}</button>)}</div>

            {activeTab==='overview'&&<div className="mt-5 space-y-4">
              <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4"><h4 className="font-bold text-indigo-950">Why this career fits</h4><p className="mt-2 text-sm leading-6 text-indigo-900">{displayReason(rec)} Your current evidence-based fit is {matchPercent(rec)}%.</p>{guidance.careerSummary&&<p className="mt-2 text-sm leading-6 text-indigo-800">{guidance.careerSummary}</p>}</div>
              <div><h4 className="font-bold">{strongCore ? 'Career advancement opportunities' : 'Priority development areas'}</h4><div className="mt-3 grid gap-3 md:grid-cols-2">{guidance.advancement.map((item)=><div key={`${item.title}-${item.detail}`} className="rounded-xl bg-white p-4"><p className="font-bold">{item.title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{item.detail}</p></div>)}</div></div>
            </div>}

            {activeTab==='market'&&<div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-white p-4"><p className="text-xs font-bold uppercase text-slate-400">Annual wage</p><p className="mt-1 text-xl font-bold">{salaryFor(rec)}</p><p className="mt-2 text-xs text-slate-500">{salarySource(rec)}</p></div><div className="rounded-xl bg-white p-4"><p className="text-xs font-bold uppercase text-slate-400">National employment</p><p className="mt-1 text-xl font-bold">{market?.bls?.available?integer(market.bls.employment):'Not available'}</p></div>{market?.onet?.available&&<div className="rounded-xl bg-white p-4 md:col-span-2"><p className="font-bold">{market.onet.occupation_title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{market.onet.description}</p></div>}<div className="rounded-xl border border-sky-100 bg-sky-50 p-4 md:col-span-2"><p className="text-sm text-sky-900">Market figures are shown only when the backend returns a confirmed BLS/O*NET mapping. Specialty careers may use a broader official SOC occupation for wage and employment statistics.</p></div></div>}

            {activeTab==='certifications'&&<div className="mt-5 space-y-5">
              {resumeCredentials.length>0&&<div><h4 className="font-bold">Credentials already evidenced</h4><p className="mt-1 text-sm text-slate-600">These credentials were found in the saved resume. Resume evidence is not the same as independent credential verification.</p><div className="mt-3 grid gap-3 md:grid-cols-2">{resumeCredentials.map((item,i)=>{const name=typeof item==='string'?item:item?.name; const provider=typeof item==='string'?'':item?.provider; return <div key={`${name}-${i}`} className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="font-bold text-emerald-950">{name}</p>{provider&&<p className="mt-1 text-sm text-emerald-800">{provider}</p>}<p className="mt-2 text-xs font-semibold text-emerald-700">Resume evidence</p></div>;})}</div></div>}
              <div><h4 className="font-bold">Recommended credentials</h4><p className="mt-1 text-sm text-slate-600">Only additional credentials suggested for this career are shown here. Existing resume credentials are not repeated as recommendations.</p>{recommendedCredentials.length?<div className="mt-3 space-y-3">{recommendedCredentials.map((item,i)=><div key={`${item.name||item}-${i}`} className="rounded-xl bg-white p-4"><p className="font-bold">{item.name||item}</p>{item.provider&&<p className="mt-1 text-sm text-slate-500">{item.provider}</p>}{item.note&&<p className="mt-2 text-sm leading-6 text-slate-600">{item.note}</p>}{item.url&&<a className="mt-3 inline-block text-sm font-semibold text-indigo-600" href={item.url} target="_blank" rel="noreferrer">Official page</a>}</div>)}</div>:<p className="mt-3 text-sm text-slate-500">No additional credential is currently suggested by this evidence model.</p>}</div>
            </div>}
            ...
            {activeTab==='resources'&&<div className="mt-5 space-y-3">{guidance.resources.map((item,i)=><div key={`${item.name||item}-${i}`} className="rounded-xl bg-white p-4"><p className="font-bold">{item.name||item}</p>{item.purpose&&<p className="mt-2 text-sm text-slate-600">{item.purpose}</p>}{item.url&&<a className="mt-3 inline-block text-sm font-semibold text-indigo-600" href={item.url} target="_blank" rel="noreferrer">Open resource</a>}</div>)}</div>}
          </div>}
        </article>;
      })}</section>
    </>}
