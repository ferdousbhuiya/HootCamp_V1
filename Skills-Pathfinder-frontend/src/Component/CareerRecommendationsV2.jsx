...
function CareerRecommendationsV2({skills,user,onBack}){
  ...
  const hasDegree=safe(skills?.education||skills?.structured_evidence?.education).some(e=>/bachelor|master|doctor|juris|ph\.?d|b\.s|m\.s/i.test(`${e?.program_or_degree||''} ${e?.field_of_study||''}`));
