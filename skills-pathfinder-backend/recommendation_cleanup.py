...
without requiring an exact wording match.
"""
...
def _normalize(value: Any) -> str:
    ...
    text = re.sub(r"\([^)]*\)", " ", text)
    text = re.sub(
        r"\b(?:license|licence|licensed|licensure|certification|certificate|certified|credential|registration|registered)\b",
        " ",
        text,
    )
...
def _acronym(value: Any) -> str:
    tokens = [token for token in _tokens(value) if token not in {"and", "of", "the", "for", "in"}]
...
def _explicit_acronyms(value: Any) -> Set[str]:
    ...
    acronyms = {
        token.lower()
        for token in re.findall(r"\b[A-Z][A-Z0-9]{1,7}\b", raw)
        if len(token) >= 2
    }
    # Also derive the acronym of a long-form phrase such as "Basic Life Support".
    derived = _acronym(raw)
    if 2 <= len(derived) <= 8:
        acronyms.add(derived.lower())
    return acronyms
...
def credentials_equivalent(left: Any, right: Any) -> bool:
    ...
    left_acronyms, right_acronyms = _explicit_acronyms(left), _explicit_acronyms(right)
    shared_acronyms = left_acronyms & right_acronyms
    if len(shared_acronyms) >= 2:
        return True
    if shared_acronyms and left_tokens and right_tokens:
